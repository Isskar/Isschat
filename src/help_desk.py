...
class HelpDesk():
    ...
    def get_llm(self):
        # Optimiser les paramètres pour des réponses plus rapides
        llm = Ollama(
            model="llama2",
            temperature=0.1,  # Réduire la température pour des réponses plus déterministes
            num_predict=512,  # Limiter la longueur de la réponse
            num_ctx=1024,     # Réduire la taille du contexte
            num_thread=4      # Utiliser plusieurs threads pour accélérer l'inférence
        )
        return llm

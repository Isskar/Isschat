...
class SemanticRetrievalTool:
    ...
    def test_vector_retrieval(self, test_query: str = "qui sont les collaborateurs sur Isschat") -> Dict[str, Any]:
        """Test vector retrieval with optional semantic re-ranking"""
        try:
            self._initialize()
            ...
            # Test with semantic re-ranking enabled
            reranked_results = self.retrieve(test_query, k=5, use_semantic_reranking=True)

            # Test with semantic re-ranking disabled (direct vector retrieval)
            direct_results = self.retrieve(test_query, k=5, use_semantic_reranking=False)
            ...
            return {
                ...
                "reranked_results": {
                    ...
                    "sample_content": reranked_results[0].content[:200] + "..." if reranked_results else None,
                },
                "direct_results": {
                    "count": len(direct_results),
                    "scores": [r.score for r in direct_results],
                    "sample_content": direct_results[0].content[:200] + "..." if direct_results else None,
                },
                "improvement": {
                    "score_improvement": (
                        (reranked_results[0].score - direct_results[0].score)
                        if reranked_results and direct_results
                        else 0
                    ),
                    "reranking_helps": (
                        len(reranked_results) > 0
                        and (not direct_results or reranked_results[0].score > direct_results[0].score)
                    ),
                },
            }

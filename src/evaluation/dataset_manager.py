...
class DatasetManager:
    ...
    def _parse_tsv_row(self, row: Dict[str, str], row_num: int) -> Optional[TestCase]:
        ...
        for key, value in row.items():
            if key not in ['id', 'question', 'expected_answer', 'expected_behavior', 
                          'test_type', 'robustness_type', 'difficulty', 'category', 'expected_sources']:
                metadata[key] = value
    ...
    def _save_tsv_dataset(self, file_path: str, test_cases: List[TestCase]):
        ...
        fieldnames = [
            'id', 'question', 'expected_answer', 'expected_behavior',
            'test_type', 'robustness_type', 'difficulty', 'category', 'expected_sources'
        ]
    ...
    def _save_json_dataset(self, file_path: str, test_cases: List[TestCase]):
        ...
        data = {
            'metadata': {
                'name': f"Isschat Evaluation Dataset",
                'version': "1.0",
                'description': f"Test cases for RAG evaluation",
    ...
    def list_all_datasets(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {
            name: self.get_dataset_info(name)
            for name in self.config.test_datasets.keys()
        }
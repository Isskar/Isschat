...
class LLMJudge:
    ...
    def _init_llm_client(self):
        ...
        try:
            ...
            self.client = openai.OpenAI(api_key=self.config.llm.api_key, base_url=self.config.llm.api_base)
    ...
    def evaluate_generation(
        self, question: str, context: str, response: str, expected_answer: Optional[str] = None
    ) -> GenerationScore:
        """
        ...
        prompt = self.config.prompts.generation_evaluation_prompt.format(
            question=question, context=context, response=response, expected_answer_section=expected_answer_section
        )
    ...
    def evaluate_robustness(
        self, test_type: RobustnessTestType, question: str, response: str, expected_behavior: str
    ) -> RobustnessScore:
        """
        ...
        if not self.client:
            ...
            return RobustnessScore(
                score=0.0, passed=False, justification="LLM client not available", test_type=test_type
            )

        # Format prompt
        prompt = self.config.prompts.robustness_evaluation_prompt.format(
            test_type=test_type.value, expected_behavior=expected_behavior, question=question, response=response
        )
        ...
        try:
            ...
            return RobustnessScore(
                score=0.0, passed=False, justification=f"Evaluation error: {str(e)}", test_type=test_type
            )
    ...
    def _parse_robustness_response(self, llm_response: str, test_type: RobustnessTestType) -> RobustnessScore:
        ...
        try:
            ...
            self.logger.debug(f"Raw LLM response: {llm_response}")
            return RobustnessScore(score=0.0, passed=False, justification=f"Parse error: {str(e)}", test_type=test_type)

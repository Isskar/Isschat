...
from .models import TestCase, EvaluationResult, EvaluationSession, BenchmarkResult, TestType, Difficulty
import sys
import os
...
class EvaluationManager:
    ...
    def create_evaluation_session(self, session_name: str, description: str, test_types: List[TestType]) -> str:
        """
    ...
    def run_evaluation(
        ...
        self.logger.info(
            f"Starting evaluation for session '{session.session_name}' with {len(all_test_cases)} test cases"
        )
            ...
            for i, test_case in enumerate(all_test_cases):
                try:
                    ...
                    if test_case.robustness_type:
                        result = self.generation_evaluator.evaluate_robustness_test(test_case, response, sources)
                    else:
    ...
    def run_robustness_tests(
        self, get_response_func: Callable[[str], tuple[str, List[str]]], session_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def create_benchmark(
        ...
        self.logger.info(
            f"Benchmark completed: {passed_tests}/{total_tests} tests passed ({benchmark_result.pass_rate:.1f}%)"
        )
    ...
    def _calculate_evaluation_summary(self, results: List[EvaluationResult], execution_time: float) -> Dict[str, Any]:
        ...
        if not results:
            return {"total_tests": 0, "execution_time": execution_time, "error": "No results to summarize"}

...
class DataLoader:
            ...
            if len(pages) > 0:
                ...
                for page in pages:
                    ...
                    try:
                        ...
                        print(
                            f"Erreur lors de la récupération des sous-pages pour {page.get('title', 'Sans titre')}: {str(e)}"  # noqa
                        )
            ...
            for i, page in enumerate(pages):
                try:
                    ...
                    page_data = confluence.get_page_by_id(
                        page_id,
                        expand="body.storage,history,space,version,ancestors,children.page,children.attachment,metadata.properties",  # noqa
                    )
    ...
    def create_dummy_docs(self):
        ...
        dummy_docs = [
            Document(
                page_content="Bienvenue dans l'assistant Confluence. Cette base de données est une version factice créée car la connexion à Confluence a échoué.",  # noqa
                metadata={"source": "dummy_doc_1", "title": "Bienvenue"},
            ),
            Document(
                page_content="Pour utiliser l'assistant avec les vraies données, vérifiez vos paramètres de connexion à Confluence dans le fichier .env.",  # noqa
                metadata={"source": "dummy_doc_2", "title": "Configuration"},
            ),
            Document(
                page_content="Assurez-vous que les variables CONFLUENCE_SPACE_NAME, CONFLUENCE_SPACE_KEY, CONFLUENCE_USERNAME et CONFLUENCE_PRIVATE_API_KEY sont correctement configurées.",  # noqa
                metadata={
                    "source": "dummy_doc_3",
